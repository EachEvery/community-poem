    ...
    <div>
        ...
        >
            <div v-if="responses.length > 0" class="flex relative">
                <div class="flex" style="transform: scale(0.7)">
                    <clickable
                        @click="() => prevIndex(responses, 'responseIndex')"
                        >👈</clickable
                    >
                    <h3
                    ...
                    <clickable
                        @click="() => nextIndex(responses, 'responseIndex')"
                        >👉</clickable
                    >
                </div>
                ...
                <h3
                    ...
                    style="width: 10rem"
                    :class="{
                        'text-green-600': response.status === 'approved',
                    }"
                >
            ...
            <h3 class="self-center uppercase" v-if="responses.length === 0">
                No {{ selectedStatus }} Responses
            </h3>

            <div class="flex mx-12">
                <div class="self-center mr-3" style="width: 5rem">
                    🔎
                    <span class="ml-2 text-sm font-light tracking-wider">{{
                        fontSize
                    }}</span>
                </div>

                <div class="flex" style="transform: scale(0.7)">
                    <clickable class="mr-1 leading-none" @click="decreaseFont"
                        >➖</clickable
                    >
                    <clickable class="mr-1 leading-none" @click="increaseFont"
                        >➕</clickable
                    >
                </div>
            </div>
            ...
            <div class="flex flex-grow justify-end">
                <clickable
                    class="mr-3"
                    @click="discard"
                    v-if="response.status === 'unapproved'"
                    >🚫 Discard</clickable
                >
                <clickable
                ...
                <clickable
                    class="mr-3"
                    v-if="response.status === 'unapproved'"
                    @click="(e) => save(e, false)"
                    >✅ Approve</clickable
                >

                <clickable
                    class="mr-3"
                    @click="() => displayOnSlideshow(true)"
                    v-if="
                        !response.display_on_slideshow &&
                        response.status === 'approved'
                    "
                    >🖥️ Display On Slideshow</clickable
                >
                <clickable
                    class="mr-3"
                    @click="() => displayOnSlideshow(false)"
                    v-if="response.display_on_slideshow"
                    >❌ Hide From Slideshow</clickable
                >
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        selectResponse(e, response) {
            ...
            this.responseIndex = this.responses.findIndex(
                (i) => i.id === response.id
            );
...
This action cannot be undone.`);
        ...
        async displayOnSlideshow(shouldDisplayOnSlideshow) {
            let { data } = await axios
                .patch(
                    `/spaces/${this.space.slug}/responses/${this.response.id}/slideshow`,
                    {
